...
$conn = $pdo->open();
$smtp =$conn->prepare("SELECT *,COUNT(*) As numrows FROM updates WHERE slug =:slug  " );
...
if (!empty($filename)) {
	...
	try {
		...
		$smtp = $conn->prepare("INSERT INTO updates(category_id,name, description,user_id, slug,writers_name, photo,date_posted,image_description) values (:category, :name, :description,:user_id, :slug,:writers_name, :photo,:date_posted,:image_description)");
